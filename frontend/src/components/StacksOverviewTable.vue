    ...
    computed: {
        ...
        aggregateStats() {
            const totals = {
                ...
                totalGpuPercent: 0,
                totalGpuVram: 0,
                totalStacks: this.stacks.length,
                hasGpu: this.hasGpu,
                gpuCards: this.gpuCards,
            };
            for (const s of this.stacks) {
                if (s.isRunning) {
                    totals.runningCount++;
                }
                totals.totalContainers += s.containers || 0;
                totals.totalCpu += s.cpu || 0;
                totals.totalMemory += s.memory || 0;
                totals.totalGpuPercent += s.gpuPercent || 0;
                totals.totalGpuVram += s.gpuVramMb || 0;
            }
            return totals;
        },
    },
